  ...
  return L.divIcon({
    ...
    html: `
      <div style="position:relative;width:32px;height:32px;display:flex;alignItems:center;justifyContent:center;">
        ...
        <div style="position:absolute;inset:2px;border-radius:50%;background:${color};box-shadow:0 0-10px ${color}88;display:flex;align-items:center;justify-content:center;color:white;font-weight:bold;font-size:10px;font-family:monospace;border:1px solid rgba(255,255,255,0.3);z-index:2;">
          ${aqi}
        </div>
      </div>
  ...
  useEffect(() => {
    ...
    stations.forEach(station => {
      const marker = L.marker([station.lat, station.lng], { icon: createMarkerIcon(station.aqi) })
        .addTo(map)
        .bindPopup(`
          <div style="font-family:'IBM Plex Mono',monospace;font-size:11px;background:#0f172a;color:#e2e8f0;padding:10px;border-radius:8px;min-width:180px;">
            <strong style="font-family:Syne,sans-serif;font-size:14px;display:block;margin-bottom:4px;">${station.name}</strong>
            <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;">
              <span style="color:${getAQIColor(station.aqi)};font-size:20px;font-weight:700;">${station.aqi}</span>
              <span style="color:${getAQIColor(station.aqi)};font-size:10px;text-transform:uppercase;">${getAQICategory(station.aqi).label}</span>
            </div>
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:4px;margin-bottom:8px;opacity:0.9;">
            ...
            <div style="border-top:1px solid rgba(255,255,255,0.1);padding-top:4px;display:flex;justify-content:space-between;color:#94a3b8;">
...
const Dashboard = () => {
  const { cityAQI, cityName, stations, lastUpdated, userCoords, viewCoords, setViewCoords, handleMapClick, dataLoading } = useAirQuality();
      ...
      <div className="flex flex-col lg:flex-row h-[calc(100vh-4rem)]">
        ...
        <div className="w-full lg:w-72 p-4 flex flex-col gap-4 flex-shrink-0">
          <div className="glass-card p-4">
            <h3 className="font-heading text-xs font-semibold text-foreground uppercase tracking-widest mb-3">Location Filter</h3>
            <div className="space-y-3">
              <div>
                <label className="text-[10px] font-mono text-muted-foreground uppercase block mb-1">State</label>
                <select 
                  value={selectedState}
                  onChange={(e) => {
                  ...
                  className="w-full bg-muted border border-border rounded-md px-3 py-1.5 text-xs font-mono text-foreground focus:ring-1 focus:ring-primary outline-none"
                >
              ...
              <div>
                ...
                <select 
                  ...
                  onChange={(e) => {
                    ...
                    const district = stateObj?.districts.find(d => d.name === distName);
                    if (district) {
                    setViewCoords([district.lat, district.lng]);
                  }
                }}
                  className="w-full bg-muted border border-border rounded-md px-3 py-1.5 text-xs font-mono text-foreground focus:ring-1 focus:ring-primary outline-none disabled:opacity-50"
                >
                  <option value="">Select District</option>
                  {indianStates.find(s => s.name === selectedState)?.districts.map(d => (
        ...
        <div className="flex-1 relative min-h-[400px]">
          ...
          >
            ...
            <LocateMeButton coords={userCoords} />
            <MapEvents onMapClick={handleMapClick} />
          </MapContainer>
          {dataLoading && (
            <div className="absolute inset-0 bg-background/20 backdrop-blur-[1px] z-[500] flex items-center justify-center">
              <div className="bg-card p-3 rounded-lg border border-border shadow-2xl flex items-center gap-3">
                <div className="w-4 h-4 border-2 border-primary border-t-transparent rounded-full animate-spin" />
                <span className="text-xs font-mono">Analyzing Location...</span>
              </div>
            </div>
          )}
        </div>
